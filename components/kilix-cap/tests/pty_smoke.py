...
def run_startup_signal_smoke(
    args: argparse.Namespace,
    pty_module: object,
    fcntl_module: object,
    termios_module: object,
) -> bool:
    """Terminate during the graphics probe and require complete PTY restore."""
    ...
    try:
        ...
        if exc.errno in SKIP_ERRNOS:
            _skip(f"PTY allocation is unavailable: {exc}")
            return False
        raise SmokeFailure(f"cannot allocate startup-signal PTY: {exc}") from exc
    ...
    try:
        ...
        environment.pop("KILIX_CAP_SKIP_PROBE", None)
        environment.pop("KITTYFB_SKIP_PROBE", None)
        environment.update({"KILIX_CAP_AUDIO": "off", "TERM": "xterm-kitty"})
        ...
        while GRAPHICS_PROBE not in output:
            if time.monotonic() >= deadline:
                raise SmokeFailure(
                    "startup-signal child never reached the graphics probe; "
                    f"output tail: {_tail(output)}"
                )
            if proc.poll() is not None:
                raise SmokeFailure(
                    "startup-signal child exited before the graphics probe; "
                    f"output tail: {_tail(output)}"
                )
            readable, _, _ = select.select([master_fd], [], [], 0.05)
            if readable and not _read_available(master_fd, output, args.output_cap):
                raise SmokeFailure("startup-signal PTY closed during probe")

        raw_termios = termios_module.tcgetattr(check_fd)  # type: ignore[attr-defined]
        if raw_termios == initial_termios:
            raise SmokeFailure("graphics probe began before the PTY entered raw mode")

        os.killpg(proc.pid, signal.SIGTERM)
        while proc.poll() is None:
            if time.monotonic() >= deadline:
                raise SmokeFailure("SIGTERM remained pending past the probe timeout")
            readable, _, _ = select.select([master_fd], [], [], 0.05)
            if readable:
                _read_available(master_fd, output, args.output_cap)

        return_code = proc.wait(timeout=max(0.01, deadline - time.monotonic()))
        if return_code == 0:
            raise SmokeFailure("startup SIGTERM unexpectedly produced a clean exit")
        ...
        if restored_termios != initial_termios:
            raise SmokeFailure("startup SIGTERM left PTY termios in raw mode")
        if (restored_flags & os.O_NONBLOCK) != (initial_flags & os.O_NONBLOCK):
            raise SmokeFailure("startup SIGTERM left PTY output nonblocking")

        print("pty_smoke: startup SIGTERM restored probe-time terminal state")
        return True
    finally:
        if proc is not None:
            _kill_child(proc)
...
def main() -> int:
    ...
    try:
        if not run_startup_signal_smoke(args, pty, fcntl, termios):
            return 0
