#!/usr/bin/env python3
"""Prepare the generated 2x2 small-prop sheet for runtime composition.

The sheet carries the Storeroom's three shelf props (storage box, wooden
crate, tin canister) and the Study laptop. Like game-media, the keyed RGBA
source is split on an invisible grid, each subject is fitted into a
48x56 cell, and the result is written as an RGB atlas plus a grayscale
coverage mask. The pair is an OPTIONAL add-on to the mandatory bundle
(src/art.c loads it both-or-neither), so this tool never touches the
thirteen mandatory runtime files.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source",
        type=Path,
        default=Path("assets/art/mansion-items.png"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("assets/art/runtime/mansion-items.png"),
    )
    parser.add_argument(
        "--mask",
        type=Path,
        default=Path("assets/art/runtime/mansion-items-mask.png"),
    )
...
def fitted_sprite(cell: Image.Image) -> Image.Image:
    ...
    if bounds is None:
        raise ValueError("mansion-items grid cell has no visible subject")
...
def main() -> int:
    ...
    print(
        f"prepare_mansion_items: {args.source} -> {args.output}, {args.mask} "
        f"({ATLAS_W}x{ATLAS_H}, four {CELL_W}x{CELL_H} sprites)"
    )
    return 0
